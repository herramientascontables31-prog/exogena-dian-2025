...
(function(){
  ...
  megaCSS.textContent=`
    ...
    @media(max-width:768px){
      ...
      .ed-nav #navLinks.open{display:flex}
      .tn-sep{display:none}
  ...
  waCSS.textContent='.whatsapp-float{position:fixed;bottom:20px;right:20px;width:60px;height:60px;background:#25D366;border-radius:50%;display:grid;place-items:center;box-shadow:0 4px 12px rgba(0,0,0,.2);z-index:9990;text-decoration:none;transition:transform .2s;animation:waBounce .6s .5s both}'+
  '.whatsapp-float:hover{transform:scale(1.1)}'+
  '.whatsapp-float svg{width:32px;height:32px;fill:#fff}'+
  '@keyframes waBounce{0%{transform:scale(0)}50%{transform:scale(1.15)}100%{transform:scale(1)}}'+
  '@media print{.whatsapp-float{display:none!important}}';
  document.head.appendChild(waCSS);
  var waBtn=document.createElement('a');
  waBtn.className='whatsapp-float';
  waBtn.href='https://wa.me/573054559574?text=Hola%2C%20tengo%20una%20pregunta%20sobre%20Exo%CC%81genaDIAN';
